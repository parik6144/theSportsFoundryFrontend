...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const record = await db.enquiry.findUnique({ where: { id: recordId } });
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const record = await db.enquiry.update({ where: { id: recordId }, data: body });
    return apiSuccess(record);
  } catch (err: any) {
    return apiDbError(err);
  }
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const recordId = parseId(id);
    if (!recordId) return apiError("Not found", 404);
    await db.enquiry.delete({ where: { id: recordId } });
