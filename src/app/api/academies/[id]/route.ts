...
export async function GET(
  ...
  try {
    ...
    const record = await repoGet("academies", id);
...
export async function PUT(
  ...
  try {
    ...
    const record = await repoUpdate("academies", id, prepareAcademyBody(body) as Record<string, unknown>);
    if (!record) return apiError("Not found", 404);
    return apiSuccess(record);
  } catch (err: any) {
    return apiDbError(err);
...
export async function DELETE(
  ...
  try {
    const { id } = await params;
    const ok = await repoDelete("academies", id);
